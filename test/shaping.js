...
describe('shaping', function () {
  ...
  describe('thai shaper', function () {
    // SARA AM (U+0E33) decomposition + NIKHAHIT reorder. Without the shaper, fontkit leaves U+0E33 unsplit and the GSUB chain rule for tone-mark shifting (uni0E49 → uni0E49.small) cannot fire.
    ...
    // Common Thai phrase exercising ccmp + mark positioning across syllables.
    test('should shape สวัสดี',
      'NotoSans/NotoSansThai-Regular.ttf', 'สวัสดี',
      '110+572|134+492|45@10,0+0|110+572|12+616|94+0');

    // Bare SARA AM at the start of a run — decomposes into NIKHAHIT + SARA AA with no preceding base to walk back over. Exercises the `target > 0` short-circuit.
    test('should decompose a bare SARA AM',
      'NotoSans/NotoSansThai-Regular.ttf', 'ำ',
      '59+0|86+406');

    // SARA AM after only an above-base mark (no consonant base). NIKHAHIT walks back over the above mark to index 0.
    ...
    // Two SARA AMs in one run — exercise the loop entering decomposition twice and re-positioning the cursor correctly between them.
    ...
    // Lao SARA AM (U+0EB3) — the script-agnostic mask handles both Thai and Lao. This requires the 'lao ' OT tag (with trailing space) in the shaper registry.
