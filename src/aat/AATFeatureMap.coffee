class AATFeatureMap
  ...
  feature = (name, selector) ->
    [features[name].code, features[name][selector]]
  ...
  @mapAATToOT: (features) ->
    ...
    if Array.isArray(features)
      for feature in features
        ...
        if r = AATMapping[f[0]]?[f[1]]
          res[r] = true
          
    else if typeof features is 'object'
      for type, feature of features
        for setting, v of feature
          f = mapFeatureStrings [type, setting]
          if v and (r = AATMapping[f[0]]?[f[1]])
            res[r] = true
